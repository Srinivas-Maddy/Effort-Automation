...
public class ListPage extends BaseAutomationPage{
	...
	public void selectListFields() {
		...
		for (int i = 0; i < listFields.size(); i++) {
			...
			hardWait(1);
			if(counter<3)
				driver.findElement(By.xpath("//table[@id='formFields']/tbody/tr["+counter+"]/td[2]/input[@type='checkbox']")).click();
			...
			if(fieldDatatype.contains("Single-select List")) 
				driver.findElement(By.xpath("//textarea[@id='entityFieldSpecs"+counter+"_fieldType_value']")).sendKeys("123");
		...
		logger.info("Starting of selectTerritory Methond");	
	      ...
	      for (int i = 0; i < this.addListFields.size(); i++) {
				...
				if(label.equalsIgnoreCase("Territory")) {
					WebElement territoryPicker=driver.findElement(By.xpath("//div[@id='s2id_fields"+i+"_fieldValue']"));
